...
API_KEY = os.getenv(
    "WEATHERAPI_KEY", "13b1b024b2794e31a07102925253003"
)  # Replace fallback with None or raise error in production
...
def configure_api(api_key: str) -> weatherapi.Configuration:
    ...
    if not api_key:
        raise ValueError(
            "API key not found. Set the WEATHERAPI_KEY environment variable."
        )
...
def fetch_forecast(
    ...
    try:
        logging.info(
            f"Fetching forecast for '{query}', days: {days}, start_date: {dt if dt else 'today'}"
        )
        ...
        logging.error(
            f"An unexpected error occurred during API call for '{query}': {e}"
        )
        return None
...
def save_forecast(data: dict, filename: str) -> bool:
    ...
    try:
        ...
        logging.error(
            f"An unexpected error occurred during saving to '{filename}': {e}"
        )
        return False
...
def main():
    parser = argparse.ArgumentParser(
        description="Fetch weather forecast data using WeatherAPI."
    )
    parser.add_argument(
        "-q",
        "--query",
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        choices=range(1, 15),  # Check API limits
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force fetch even if a recent file exists."
    )  # Simple force flag
    ...
    if not args.force and os.path.exists(filepath):
        logging.info(
            f"Forecast file '{filepath}' already exists. Use --force to overwrite."
        )
